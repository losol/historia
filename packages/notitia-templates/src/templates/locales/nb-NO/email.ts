...
<body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; background-color: ${emailColors.pageBackground};">
  <div style="max-width: 600px; margin: 40px auto; background-color: ${emailColors.cardBackground}; border-radius: 8px; overflow: hidden; box-shadow: 0 2px 8px rgba(0, 0, 0, 0.1);">
    <div style="background: ${emailColors.primary}; color: ${emailColors.onPrimary}; padding: 32px; text-align: center;">
      <h1 style="margin: 0; font-size: 24px; font-weight: 600;">{{appName}} - Innloggingskode</h1>
    </div>
    <div style="padding: 40px 32px; text-align: center;">
      <p style="color: ${emailColors.textMuted}; font-size: 16px; line-height: 1.5; margin: 0 0 24px 0;">
        Din engangs innloggingskode er:
      </p>
      <div style="background-color: ${emailColors.panelBackground}; border: 2px solid ${emailColors.border}; border-radius: 8px; padding: 24px; margin: 32px 0;">
        <div style="font-size: 36px; font-weight: 700; letter-spacing: 8px; color: ${emailColors.text}; font-family: 'Courier New', monospace;">
          {{code}}
        </div>
        <p style="color: ${emailColors.textMuted}; font-size: 14px; margin-top: 16px;">
          Denne koden utløper om {{expiresInMinutes}} minutter.
        </p>
      </div>
      <div style="background-color: ${emailColors.warningBackground}; border-left: 4px solid ${emailColors.warning}; padding: 16px; margin: 24px 0; text-align: left;">
        <p style="margin: 0; color: ${emailColors.warningText}; font-size: 14px;">
          Hvis du ikke ba om denne koden, kan du trygt ignorere denne e-posten.
        </p>
      </div>
    </div>
    <div style="background-color: ${emailColors.panelBackground}; padding: 24px 32px; text-align: center; border-top: 1px solid ${emailColors.border};">
      <p style="margin: 0; color: ${emailColors.textMuted}; font-size: 12px;">
        Dette er en automatisk melding, vennligst ikke svar.
      </p>
      <p style="margin: 8px 0 0 0; color: ${emailColors.textMuted}; font-size: 12px;">
        &copy; {{year}} {{appName}}
      </p>
    </div>
  </div>
