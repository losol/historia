      ...
      <Grid>
        {navigation.map((navBlock, index) => (
          <div key={navBlock.id || index}>
            {navBlock.title && (
              <Heading as="h3" className="pb-2 text-sm font-semibold uppercase" padding="none">
                {navBlock.title}
              </Heading>
            )}
            <List as="ul" variant="unstyled" className="space-y-2 text-gray-600 dark:text-gray-400">
              {navBlock.items?.map((item, itemIndex) => {
                if (item.blockType === 'separator') {
                  ...
                  if (item.style === 'dots') {
                    return (
                      <List.Item key={item.id || itemIndex} className="text-center text-gray-400">
                        • • •
                      </List.Item>
                    );
                  }
