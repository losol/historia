          ...
          <div key={order.id} className={styles.orderCard}>
            ...
            <div className={styles.customerInfo}>
              ...
              {typeof order.customer === 'object' &&
                order.customer &&
                'phone_number' in order.customer &&
                order.customer.phone_number && (
                  <p className={styles.customerPhoneLine}>
                    <strong>Phone:</strong> {formatPhoneForDisplay(order.customer.phone_number)}
                  </p>
                )}
            </div>
            ...
            <table className={styles.itemsTable}>
              <thead>
                <tr>
                  <th className={`${styles.itemsHeaderCell} ${styles.itemsHeaderCellLeft}`}>
                    Product
                  </th>
                  <th className={`${styles.itemsHeaderCell} ${styles.itemsHeaderCellRight}`}>
                    Qty
                  </th>
                  <th className={`${styles.itemsHeaderCell} ${styles.itemsHeaderCellRight}`}>
                    Total
                ...
                {(order.items ?? []).map((item) => (
                  <tr key={item.itemId}>
                    ...
                    <td className={`${styles.itemsCell} ${styles.itemsCellRight}`}>
                      ...
                      {(item.lineTotal
                        ? item.lineTotal / 100
                        : (item.price.amountExVat * item.quantity) / 100
                      ).toFixed(2)}
                    </td>
                  </tr>
                ))}
              ...
              <tfoot>
                <tr>
                  <td colSpan={2} className={styles.itemsFooterLabel}>
                    <strong>Total:</strong>
                  </td>
                  <td className={styles.itemsFooterValue}>
                    <strong>
                      {order.currency} {((order.totalAmount ?? 0) / 100).toFixed(2)}
                    </strong>
                  </td>
                </tr>
              </tfoot>
